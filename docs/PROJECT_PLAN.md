...
**Client:** Croma (Jared Hasson, Admin)  
...
## 2. Admin's Core Workflow (Priority Order)

Based on Admin's feedback, the internal user workflow priorities are:
...
| # | Feature | Hours | Description | Admin Priority |
|---|---------|-------|-------------|-----------------|
...
│                               │                               │
├─ M1: Login Demo ──────────────├─ M2: Admin Workflow ─────────├─ M3: MVP Complete
│  (Hour 3)                     │  (Hour 7)                     │  (Hour 10)
│                               │                               │
│  CHECKPOINT 1                 │  CHECKPOINT 2                 │  CHECKPOINT 3
│  Discord call w/ Admin       │  Discord call w/ Admin       │  Final review w/ Jared
...
- [ ] Admin receives magic link email
- [ ] Admin logs in and sees her dashboard (empty state)
...
### Milestone 2: Admin's Core Workflow (Hours 4-7)
...
**Demo to Stakeholders:**
- [ ] Admin sees test orders in dashboard
- [ ] Admin filters by status/language/date
- [ ] Admin generates invoice for an order
- [ ] Admin sends invoice (receives email)
- [ ] Admin comments on order, customer sees it
...
**Go/No-Go:** Admin confirms workflow works before building customer side
...
- [ ] Order appears in Admin's dashboard immediately
...
| Task | Hours | Priority |
|------|-------|----------|
| Proof of payment upload | 0.75 | High (Admin requested) |
...
|------------|------|-----|--------|----------|---------|
| **CP1** | After Hour 3 | Admin | Discord call | 15 min | Validate auth & navigation |
| **CP2** | After Hour 7 | Admin | Discord call | 30 min | Validate internal workflow |
| **CP3** | After Hour 10 | Admin + Jared | Discord call | 30 min | MVP sign-off |
...
| Invoice format wrong | Checkpoint 2 | Get sample invoice from Admin |
...
| Stuck for > 30 min | DM Jared/Admin for clarification |
...
- **Participants:** Admin (end-user), Jared Hasson (decision-maker)
...
- [ ] **Admin can see all orders in real-time dashboard** (Priority #1)
- [ ] **Admin can generate and send invoices in-app** (Priority #2)
- [ ] **Admin can chat with customers about payment proof** (Priority #3)
- [ ] **Admin can message customers with real names** (Priority #4)
...
2. **Discord Confirmation:** Align Admin on MVP feature set
